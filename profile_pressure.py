...
class F4TApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Watlow F4T Master - Loop 2 (Pressure)")
    ...
    def setup_ui(self):
        ...
        self.pv_label = ttk.Label(top, text="PV2: --.-", font=('Arial', 10, 'bold'), foreground="blue")
    ...
    def on_table_click(self, event):
        ...
        if col_idx == 1: # Type Column -> Combobox
            ...
            self.active_editor.bind("<<ComboboxSelected>>", lambda e: (
                setattr(self, "_pending_val", self.active_editor.get()),
                self.finish_edit(item_id, col_idx)
            ))
        elif col_idx == 4: # Rate / End Action column
    ...
    def on_focus_out(self, event):
        # Comboboxes lose focus when the dropdown opens - don't destroy them
        if isinstance(getattr(self, 'active_editor', None), ttk.Combobox):
            return
    ...
    def finish_edit(self, item_id, col_idx):
        try:
            # Use pre-captured value for End Action combobox (editor may be gone by now)
            val = getattr(self, '_pending_val', None) or self.active_editor.get()
            ...
            elif col_idx == 2: # SETPOINT (Float) - Loop 2 (pressure), Step Set Point 2 = reg 18948
                self.write_f4t_float(18948, float(val))
            
            elif col_idx == 3: # DURATION (H:M:S)
                h, m, s = map(int, val.split(':'))
                self.safe_write(18928, h); self.safe_write(18930, m); self.safe_write(18932, s)
            
            elif col_idx == 4: # RAMP RATE (Float) - Loop 2 (pressure), Step Rate 2 = reg 18940
                step_type = self.tree.item(item_id, "values")[1]
                if step_type == "End":  # End Action -> write reg 19032
                    end_action_codes = {"User": 100, "Off": 62, "Hold": 47}
                    self.safe_write(19032, end_action_codes.get(val, 100))
                else:
                    self.write_f4t_float(18940, float(val))
    ...
    def fetch_profile_table(self):
        ...
        for i in range(total_steps):
            ...
            time.sleep(0.04) 
            data = self.safe_read(18926, 24)  # Read 24 regs to reach Loop 2 setpoint at offset 22
            if data and not data.isError():
                r = data.registers
                t_val, t_name = r[0], self.type_map.get(r[0], f"Code {r[0]}")
                duration = f"{r[2]:02}:{r[4]:02}:{r[6]:02}"
                # Loop 2 (pressure): Rate 2 at offset 14 (reg 18940), Set Point 2 at offset 22 (reg 18948)
                rate   = self.decode_f4t_float(r[14:16])
                target = self.decode_f4t_float(r[22:24])
                ...
                if t_val == 27:  # End step - read End Step Loop Action 1 from reg 19030
                    end_action_names = {100: "User", 62: "Off", 47: "Hold"}
                    ea_res = self.safe_read(19032, 1)
                    d_rate = end_action_names.get(ea_res.registers[0], "User") if ea_res and not ea_res.isError() else "User"
    ...
    def poll_loop(self):
        ...
        if self.client and self.client.connected:
            pv_res = self.safe_read(2980, 2)  # Loop 2 PV - CLP2 instance 2, member 66
            if pv_res and hasattr(pv_res, 'registers'):
                self.pv_label.config(text=f"PV2: {self.decode_f4t_float(pv_res.registers):.1f}")
